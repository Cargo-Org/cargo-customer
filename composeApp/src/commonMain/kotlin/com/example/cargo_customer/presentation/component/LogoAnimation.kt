...
@Composable
private fun TruckChassis(offsetX: Float, offsetY: Float, modifier: Modifier = Modifier) {
    ...
    Canvas(modifier = modifier) {
        ...
        withTransform({
            ...
            drawPath(pathCarBody, color = White)
        }
    }
}

@Composable
private fun SpeedTrails(modifier: Modifier = Modifier) {
    val paths = remember {
        listOf(
            parsePath(D_TRAIL_1),
            parsePath(D_TRAIL_2),
        ...
        withTransform({ scale(sx, sy, pivot = Offset.Zero) }) {
            paths.forEachIndexed { index, path ->
                ...
                withTransform({ translate(left = x.value) }) {
                    drawPath(path, Orange, alpha = a.value)
                }
            }
        }
    }
...
private fun TruckCargo(
    ...
    Canvas(modifier = modifier) {
        ...
        withTransform({
            ...
            drawPath(pathCargo, Orange)
        }
    }
}

@Composable
private fun Wheels(modifier: Modifier = Modifier) {
    ...
    Canvas(modifier = modifier) {
        ...
        withTransform({ scale(sx, sy, pivot = Offset.Zero) }) {
            drawPath(pathWheelRear, White)
            drawPath(pathWheelFront, White)
        }
    }
}
